...
def build_fewshot_dataset(set_id, root, transform, mode='train', n_shot=None):
    ...
    # Resolve json path robustly to handle different dataset layouts
    if os.path.isabs(json_path):
        candidate_json_paths = [json_path]
    else:
        candidate_json_paths = [
            os.path.join(root, json_path),  # expected location (dataset root)
            os.path.join(root, os.path.basename(json_path)),  # fallback if value already contains folders
            os.path.join(root, os.path.basename(root), json_path),  # some archives nest another folder
        ]
    json_path_resolved = None
    for cand in candidate_json_paths:
        if os.path.exists(cand):
            json_path_resolved = cand
            break
    if json_path_resolved is None:
        raise FileNotFoundError(f"JSON split file not found for {set_id}: tried {candidate_json_paths}")

    return BaseJsonDataset(image_path, json_path_resolved, mode, n_shot, transform)
